        ...
        if (createUserResult) {         
            if (process.env.SECRET) {
                ...
                let mail = {
                    ...
                    html: `<p>Do not share this code with anyone, this code will expire within 5 minutes. enter the following 4 digit code to activate your account: <br>${fourDigitNumber}</p>`,
                }
                transporter.sendMail(mail, (err) => {
                    if (err) {
                        ...
                        return res.status(200).json({ msg: "User has been pre-created" });
                    }
                })
            }
        } else {
